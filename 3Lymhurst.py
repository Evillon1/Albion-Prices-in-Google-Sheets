...
class Albi(object):
    ...
    def mini_price(item_index):
        return requests.get(Albi._url('/{item_index}?locations=Lymhurst'.format(item_index=item_index))).json()

class create(Albi):
    def thetford(coll, roww, item_index):
        requests.get(Albi._url('/{item_index}?locations=Lymhurst'.format(item_index=item_index))).json()
...
thread1 = Thread(target=set.res, args=('WOOD', 34))
thread2 = Thread(target=set.res, args=('PLANKS', 36))
thread3 = Thread(target=set.res, args=('HIDE', 38))
thread4 = Thread(target=set.res, args=('LEATHER', 40))
thread5 = Thread(target=set.res, args=('ORE', 42))
thread6 = Thread(target=set.res, args=('METALBAR', 44))
