...
class create(Albi):
    def thetford(coll, roww, item_index):
        requests.get(Albi._url('/{item_index}?locations=Swamp Cross'.format(item_index=item_index))).json()
...
thread1 = Thread(target=set.res, args=('WOOD', 79))
thread2 = Thread(target=set.res, args=('PLANKS', 81))
thread3 = Thread(target=set.res, args=('HIDE', 83))
thread4 = Thread(target=set.res, args=('LEATHER', 85))
thread5 = Thread(target=set.res, args=('ORE', 87))
thread6 = Thread(target=set.res, args=('METALBAR', 89))
