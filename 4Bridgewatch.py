...
class create(Albi):
    def thetford(coll, roww, item_index):
        requests.get(Albi._url('/{item_index}?locations=Bridgewatch'.format(item_index=item_index))).json()
...
thread1 = Thread(target=set.res, args=('WOOD', 49))
thread2 = Thread(target=set.res, args=('PLANKS', 51))
thread3 = Thread(target=set.res, args=('HIDE', 53))
thread4 = Thread(target=set.res, args=('LEATHER', 55))
thread5 = Thread(target=set.res, args=('ORE', 57))
thread6 = Thread(target=set.res, args=('METALBAR', 59))
