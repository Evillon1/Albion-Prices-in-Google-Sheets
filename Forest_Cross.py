...
class Albi(object):
    ...
    def mini_price(item_index):
        return requests.get(Albi._url('/{item_index}?locations=Forest Cross'.format(item_index=item_index))).json()

class create(Albi):
    def thetford(coll, roww, item_index):
        requests.get(Albi._url('/{item_index}?locations=Forest Cross'.format(item_index=item_index))).json()
...
thread1 = Thread(target=set.res, args=('WOOD', 109))
thread2 = Thread(target=set.res, args=('PLANKS', 111))
thread3 = Thread(target=set.res, args=('HIDE', 113))
thread4 = Thread(target=set.res, args=('LEATHER', 115))
thread5 = Thread(target=set.res, args=('ORE', 117))
thread6 = Thread(target=set.res, args=('METALBAR', 119))
